...
<main class="p-2 sm:px-2 flex-1 overflow-auto">
    ...
    <div class="flex gap-4 mb-6">
        <label class="flex items-center gap-2 cursor-pointer">
          <button type="button" onclick="document.getElementById('buyCryptoModal').classList.remove('hidden')" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded w-full text-center">
    Buy
</button>
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            <button type="button" onclick="document.getElementById('sellCryptoModal').classList.remove('hidden')"
    ...
    <div class="space-y-4">
        ...
        <!-- Buy Crypto Modal -->
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden" id="buyCryptoModal">
    <div class="bg-gray-900 rounded-lg shadow-lg max-w-lg w-full p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-semibold text-white">Buy Cryptocurrency</h3>
            <button onclick="document.getElementById('buyCryptoModal').classList.add('hidden')" class="text-gray-400 hover:text-white">
                ✖
            </button>
        </div>
        ...
        <form action="{{ route('user.crypto.buy.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="buy_cryptoSelect" class="text-sm text-gray-300 block">Select Cryptocurrency</label>
                <div class="relative flex items-center bg-gray-800 rounded-md">
                    <img id="buy_cryptoIcon" src="" class="w-8 h-8 ml-3" alt="Crypto Icon">
                    <select id="buy_cryptoSelect" name="currency" class="w-full p-3 pl-12 bg-transparent text-white rounded-md focus:ring-2 focus:ring-green-500" required>
            ...
            <div>
                <label for="buy_cryptoAmount" class="text-sm text-gray-300 block">Amount to Buy (Crypto)</label>
                <input type="number" id="buy_cryptoAmount" name="crypto_amount" step="0.00000001" min="0"
                    class="w-full p-3 rounded-md bg-gray-800 text-white focus:ring-2 focus:ring-green-500"
                    placeholder="Enter crypto amount" required>
            </div>

            <div>
                <label for="buy_fiatAmount" class="text-sm text-gray-300 block">Total Cost (USD)</label>
                <div class="flex items-center gap-2">
                    <input type="text" id="buy_fiatAmount" name="fiat_amount" readonly
                        class="flex-1 p-3 rounded-md bg-gray-800 text-white">
                    <span class="text-white">USD</span>
                </div>
            </div>

            <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-md transition duration-200">
                Confirm Purchase
            </button>
        </form>
    </div>
...
<!-- Sell Crypto Modal -->
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden" id="sellCryptoModal">
    <div class="bg-gray-900 rounded-lg shadow-lg max-w-lg w-full p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-semibold text-white">Sell Cryptocurrency</h3>
            <button onclick="document.getElementById('sellCryptoModal').classList.add('hidden')" class="text-gray-400 hover:text-white">
                ✖
            </button>
        </div>
        ...
        <form action="{{ route('user.crypto.sell.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="sell_cryptoSelect" class="text-sm text-gray-300 block">Select Cryptocurrency</label>
                <div class="relative flex items-center bg-gray-800 rounded-md">
                    <img id="sell_cryptoIcon" src="" class="w-8 h-8 ml-3" alt="Crypto Icon">
                    <select id="sell_cryptoSelect" name="currency" class="w-full p-3 pl-12 bg-transparent text-white rounded-md focus:ring-2 focus:ring-red-500" required>
                        @foreach($currencies as $crypto)
                            <option value="{{ $crypto->symbol }}"
                                    data-icon="https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/svg/color/{{ strtolower($crypto->symbol) }}.svg">
                                {{ $crypto->name }} ({{ $crypto->symbol }})
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                <label for="sell_cryptoAmount" class="text-sm text-gray-300 block">Amount to Sell (Crypto)</label>
                <input type="number" id="sell_cryptoAmount" name="crypto_amount" step="0.00000001" min="0"
                    class="w-full p-3 rounded-md bg-gray-800 text-white focus:ring-2 focus:ring-red-500"
                    placeholder="Enter crypto amount" required>
            </div>

            <div>
                <label for="sell_fiatAmount" class="text-sm text-gray-300 block">You will receive (USD)</label>
                <div class="flex items-center gap-2">
            ...
            <button type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white font-bold py-3 px-4 rounded-md transition duration-200">
                Confirm Sale
            </button>
        </form>
    </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('DOMContentLoaded', function() {
    // Initialize both select elements
    const cryptoSelects = ['f2c_cryptoSelect', 'buy_cryptoSelect'];

    cryptoSelects.forEach(selectId => {
        const select = document.getElementById(selectId);
        const icon = document.getElementById(selectId.replace('Select', 'Icon'));

        // Set initial icon
        if (select && icon) {
            const initialOption = select.options[select.selectedIndex];
            icon.src = initialOption.getAttribute('data-icon');

            // Update icon when selection changes
            select.addEventListener('change', function() {
                const selectedOption = this.options[this.selectedIndex];
                icon.src = selectedOption.getAttribute('data-icon');

                // Update symbol in fiat to crypto modal
                if (selectId === 'f2c_cryptoSelect') {
                    document.getElementById('f2c_cryptoSymbol').textContent = selectedOption.value;
                }
            });
        }
    });
    ...
    const fiatAmountInput = document.getElementById('f2c_fiatAmount');
    if (fiatAmountInput) {
        fiatAmountInput.addEventListener('input', function() {
            // In a real app, you would fetch the current price from an API
            const cryptoSelect = document.getElementById('f2c_cryptoSelect');
            ...
            const rate = mockRates[selectedCrypto] || 1;
            const cryptoAmount = fiatAmount / rate;

            document.getElementById('f2c_cryptoAmount').value = cryptoAmount.toFixed(8);
        });
    }

    // Buy Crypto Conversion (crypto to fiat)
    const cryptoAmountInput = document.getElementById('buy_cryptoAmount');
    if (cryptoAmountInput) {
        cryptoAmountInput.addEventListener('input', function() {
            // In a real app, you would fetch the current price from an API
            const cryptoSelect = document.getElementById('buy_cryptoSelect');
            const selectedCrypto = cryptoSelect.value;
            const cryptoAmount = parseFloat(this.value) || 0;

            // Mock conversion rates - same as above
            const mockRates = {
                'BTC': 50000,
                'ETH': 3000,
                'USDT': 1
            ...
            document.getElementById('buy_fiatAmount').value = fiatAmount.toFixed(2);
        });
    }
});

// selling
// Sell Crypto Conversion
document.addEventListener('DOMContentLoaded', function() {
    // Initialize sell select element
    const sellSelect = document.getElementById('sell_cryptoSelect');
    if (sellSelect) {
        const sellIcon = document.getElementById('sell_cryptoIcon');
        const sellBalance = document.getElementById('sell_availableBalance');

        // Set initial icon and balance
        const initialOption = sellSelect.options[sellSelect.selectedIndex];
        sellIcon.src = initialOption.getAttribute('data-icon');
        sellBalance.textContent = `Available: ${initialOption.getAttribute('data-balance')} ${initialOption.value}`;

        // Update when selection changes
        sellSelect.addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            sellIcon.src = selectedOption.getAttribute('data-icon');
            sellBalance.textContent = `Available: ${selectedOption.getAttribute('data-balance')} ${selectedOption.value}`;
        });
    }

    // Sell amount conversion
    const sellCryptoInput = document.getElementById('sell_cryptoAmount');
    if (sellCryptoInput) {
        sellCryptoInput.addEventListener('input', function() {
            const cryptoSelect = document.getElementById('sell_cryptoSelect');
            const selectedCrypto = cryptoSelect.value;
            const cryptoAmount = parseFloat(this.value) || 0;

            // Get price (replace with actual API call)
            const mockRates = {
                'BTC': 50000,
                'ETH': 3000,
                'USDT': 1
            ...
            document.getElementById('sell_fiatAmount').value = fiatAmount.toFixed(2);
        });
    }
});
                  </script>
