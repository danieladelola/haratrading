...
<main class="p-2 sm:px-2 flex-1 overflow-auto bg-black text-white">
    <div class="grid blued grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        @foreach($copy_experts as $expert)
            <div class="flex justify-center">
                <div class="w-full max-w-sm bg-gray-900 rounded-xl shadow-lg shadow-gray-800 p-6 sm:flex sm:items-center sm:space-x-6">
                    ...
                    <div class="text-center sm:text-left space-y-2">
                        ...
                        <button class="px-4 py-2 text-sm sm:text-base text-purple-400 font-semibold rounded-full border border-purple-400 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2 transition">
                            Copy Expert
                        </button>
                        </form>
                    </div>
    ...
    <div id="depositModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" style="display: none;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Only show modal if user hasn't paid (you can trigger this conditionally)
            // For example, when trying to copy an expert without payment
            function showDepositModal() {
                depositModal.style.display = 'flex';
            }

            // Example of when to show the modal (you can call this function when needed)
            // showDepositModal();
        });
    </script>
</main>
