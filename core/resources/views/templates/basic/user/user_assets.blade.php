            ...
            <div class="container mx-auto p-4">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
                        <div class="flex items-center space-x-2 mb-4">
                            <h2 class="text-gray-100">Total Balance</h2>
                        ...
                        <p class="text-3xl font-bold text-white">{{ showAmount(auth()->user()->balance) }}</p>
                    </div>

                    <!-- Recent Activity -->
                    <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
                        <h2 class="text-gray-100 mb-4">Recent Activity</h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                            @foreach($topAssets as $asset)
                                ...
                                <div class="bg-gray-700 border border-gray-600 rounded-lg p-4 hover:bg-gray-600 transition-colors">
                                    ...
                                    <h3 class="text-white">{{ $asset->currency }}</h3>
                                    <p class="text-white font-medium">{{ number_format($asset->amount, 0, '.', ',') }}</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="mt-8">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-lg text-white">All your assets</h2>
                        <div class="relative">
                            <input type="text" placeholder="Search for assets"
                                class="bg-gray-700 border border-gray-600 rounded-lg pl-10 pr-4 py-2 w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400">
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-700">
                        <table class="min-w-full divide-y divide-gray-700 bg-gray-800 text-gray-100">
                                ...
                                @foreach($assets as $asset)
                                    ...
                                    <tr class="hover:bg-gray-700 transition-colors duration-200">
                                        <td class="py-3 px-4 flex items-center space-x-2">
                                            ...
                                            <span class="text-white">{{ $asset->currency }}</span>
                                        </td>
                                        <td class="py-3 px-4 text-gray-300">{{ $asset->type }}</td>
                                        <td class="py-3 px-4">
                                            <span class="text-blue-400">${{ number_format($price, 2) }}</span>
                                            @if($price == 0)
                                                <span class="text-xs text-red-400">(Unavailable)</span>
                                            @endif
                                        </td>
                                        <td class="py-3 px-4">
                                            <span class="text-white">{{ $asset->amount }} {{ $asset->currency }}</span>
                                            <span class="block text-gray-400 text-sm">
                                                {{ $price > 0 ? '$' . number_format($asset->amount * $price, 2) : 'Unavailable' }}
                                            </span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <div class="flex space-x-2">
                                                ...
                                                <a href="{{ route('user.withdraw') }}"
                                                    class="px-4 py-1 bg-gray-600 text-white rounded-lg hover:bg-gray-500 transition duration-200">
                                                    Withdraw</a>
                                            </div>
                                        </td>
                                    </tr>
